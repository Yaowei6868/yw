#!/bin/bash
# run_elliptic_cl_baselines.sh — Elliptic CL 基线实验
# 用法: nohup bash scripts/run_elliptic_cl_baselines.sh > logs/elliptic_cl_baselines.log 2>&1 &
#
# 运行 CGNN backbone 上的标准 CL 策略：EWC / LwF / ER
# 与 TASD-CL 对比，验证我们的方法优于通用 CL 策略
...
echo ""
echo "╔══════════════════════════════════════════════════════════╗"
echo "║  Elliptic CL 基线：EWC / LwF / ER on CGNN               ║"
printf "║  %-56s║\n" "$(date '+%Y-%m-%d %H:%M:%S')"
echo "╚══════════════════════════════════════════════════════════╝"

run_experiment "configs/ours/cl_on_cgnn/elliptic_EWC_CGNN.yaml"
run_experiment "configs/ours/cl_on_cgnn/elliptic_LwF_CGNN.yaml"
run_experiment "configs/ours/cl_on_cgnn/elliptic_ER_CGNN.yaml"
