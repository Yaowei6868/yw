#!/bin/bash
# run_actor_tuning_r4.sh — Elliptic++ Actor 第四轮调参
# 用法: nohup bash scripts/run_actor_tuning_r4.sh > logs/actor_tuning_r4.log 2>&1 &
#
# 基于第三轮结论：
#   - nospc_e30 是 TASD-CL 方向局部最优（F1=0.3502）
#   - r3 所有 ewc/scd 参数变体全面退步或崩溃
#   - 差距来源：GradGNN 后期 Recall 更高（0.59 vs 0.53）
#
# 第四轮探索未尝试方向：
#   - hidden_dim=256（GradGNN h256 从 0.3555→0.3679，+3.5%）
#   - threshold=0.4（降低阈值，提高 Recall 应对极度不平衡）
#   - scd_tau=0.3（更严格蒸馏过滤）
#   - h256 + lr5 组合（对齐 GradGNN 最优超参）
#
# 共 4 个实验
