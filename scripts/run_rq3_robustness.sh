...
#   FORCE_RERUN=1 nohup bash scripts/run_rq3_robustness.sh > logs/rq3_robustness.log 2>&1 &
...
run_experiment() {
    local config_path="$1"
    local exp_name save_dir csv_path log_file t_start t_end elapsed row_count
    ...
    if [[ "${FORCE_RERUN:-0}" != "1" && -f "$csv_path" ]]; then
        ...
        if [[ "$row_count" -ge "$COMPLETE_ROWS_MIN" ]]; then
            echo "  [SKIP] ${exp_name}"
...
echo ""
echo "  -- Anomaly-ratio robustness --"
for config in configs/elliptic/robustness/elliptic_robust_ratio*.yaml; do
    run_experiment "$config"
done

echo ""
echo "  -- Temporal-window robustness --"
for config in configs/elliptic/robustness/elliptic_robust_window*.yaml; do
    run_experiment "$config"
done
