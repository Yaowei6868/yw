#!/bin/bash
# TASD-CL hyperparameter sensitivity on Elliptic.
#
# Usage:
#   bash scripts/run_hyperparam_tasdcl.sh
#   FORCE_RERUN=1 nohup bash scripts/run_hyperparam_tasdcl.sh > logs/hyperparam_tasdcl.log 2>&1 &
...
echo "============================================================"
echo " TASD-CL hyperparameter sensitivity"
printf " %s\n" "$(date '+%Y-%m-%d %H:%M:%S')"
echo "============================================================"

for config in configs/elliptic/hyperparam/*.yaml; do
    run_hp "$config"
done
